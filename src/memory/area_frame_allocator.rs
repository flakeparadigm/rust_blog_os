...
impl FrameAllocator for AreaFrameAllocator {
    fn allocate_frame(&mut self) -> Option<Frame> {
        if let Some(area) = self.current_area {
            // "Cone" the frame to return it if it's free. Frame doesn't implement Clone,
            // but we can construct an identical frame.
            let frame = Frame{ number: self.next_free_frame.number };

            // the last frame of current area
            // (cool rust thing - block returns value from last line to binding)
